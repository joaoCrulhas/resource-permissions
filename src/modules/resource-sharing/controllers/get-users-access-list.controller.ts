...
export class GetUsersAccessListController implements GetUsersAccessListControllerType {
  ...
  async handle(request: GetUsersAccessListRequestDto): Promise<HttpResponse<UserEntity[]>> {
    const response = await this.getUsersAccessList.getUsersAccessList(request.resourceId);
    return {
      body: response,
